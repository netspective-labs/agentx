...
export class OmbTag {
  ...
  constructor(tagName) {
     ...
     * Expression is evaluated at runtime with variables raw, tag, z, splitCsv.
     * @type {string | undefined}
     */
 ...
 * or from an XML source referenced by `omb:src`.
 */
export class ObjectModelBuilderElement extends HTMLElement {
   ...
   * NOTE: called only for live-DOM builds (no omb:src).
   ...
   * - If `omb:src` is present, fetch+parse XML and build from its documentElement.
   * - Otherwise, build from this element's live DOM subtree.
   *
   * Dispatches:
   * - "omb:built" with { model }
   * - "omb:error" with { error } if fetch/parse fails
   *
   * @returns {Promise<OmbNode>}
   */
  async rebuild() {
    try {
      const src = this.getAttribute("omb:src");
      if (src && String(src).trim().length > 0) {
        const rootEl = await this.#loadXmlRootElement(String(src));
        const root = this.#makeNodeForElement(rootEl);
        this.#applyAttributesToNode(rootEl, root);
        this.#walkChildNodes(rootEl, root, true, [root[".tag"]]);
   ...
   */
  async #loadXmlRootElement(src) {
    const url = this.#resolveSrcUrl(src);
  ...
  #resolveSrcUrl(src) {
    ...
    if (typeof resolver === "function") return resolver(s, this);

    // Default: resolve against the owning document baseURI.
    const base = this.ownerDocument?.baseURI ?? document.baseURI;
