...
export class OmbBuilder {
  ...
  buildFromDomRoot(rootEl, ctx = {}) {
    ...
    this.#walkChildNodes(rootEl, root, true, [root[".tag"]], host);
  ...
  #walkChildNodes(domParent, modelParent, isRoot, parents, host) {
    ...
    for (const node of childNodes) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        const childModel = this.#makeNodeForElement(el, host);
  ...
  #addExistingChildModel(parentModel, childModel) {
    ...
    if (typeof adder !== "function") {
      adder = (element) =>
        this.#makeNodeForElement(element, childModel[".host"] ?? element);
    ...
    Object.defineProperty(model, key, {
      ...
      get() {
        ...
        const fn = (/** @type {any} */ (host))?.options?.typedValue ??
          defaultTypedValue;
        // If host is not an ObjectModelBuilderElement, fall back to builder defaultTypedValue
        return typeof fn === "function"
          ? fn(raw, self[".tag"])
          : defaultTypedValue(raw, self[".tag"]);
      },
      set(v) {
        const self = /** @type {OmbNode} */ (this);
