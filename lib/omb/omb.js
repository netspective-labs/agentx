...
export class ObjectModelBuilderElement extends HTMLElement {
  ...
  /**
   * Compile an omb:type-as expression into an OmbTypedValueFn (cached per expression).
   * Expression is executed as: eval(`(${EXPR})`)
   *
   * In-scope variables inside EXPR:
   * - raw, tag (locals)
   * - any module-scope bindings (e.g. z, splitCsv, defaultTypedValues, etc.)
   *
   * If evaluation throws, the thrown exception object is returned as the value.
   *
   * @param {string} expr
