...
//
// Usage:
//   deno run -A ai-context.ts > .ai-context.txt
//   deno run -A ai-context.ts --root . --no-hash
//
// Notes:
...
const FILES = [
  ...
  "./support/assurance/counter/counter-element.ts",
...
const PROJECT_BRIEF = [
  ...
  "- This repository is a Deno TypeScript codebase focused on modern web UI composition.",
  "- The approach emphasizes type-safe, testable Web Components / Custom Elements and a fluent HTML writer style.",
  "- The goal of this prompt is to provide enough context for an AI to reason about the code and propose accurate changes.",
].join("\n");
...
class AiContextBuilder {
  ...
  async build() {
    ...
    lines.push(
      "You are an AI assistant helping with a Deno TypeScript repository.",
      "Follow existing code style and be careful about deterministic, reproducible output.",
      "",
      PROJECT_BRIEF,
      "",
      "Relevant files (verbatim):",
    );
    ...
    lines.push(
      ...
      "- If you propose changes, be specific: file paths, edits, and rationale.",
      "- Prefer Deno/JSR imports, strict TypeScript, and lint-friendly code.",
    );
